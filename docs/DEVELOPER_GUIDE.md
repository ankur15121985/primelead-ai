...
- **Browser E2E:** `client/e2e/funnel.spec.ts` + `client/playwright.config.ts` — Playwright boots the **production build** (webServer: `prisma migrate deploy && seed && node dist/index.js` against a scratch `e2e.db`) and walks signup → create lead → schedule follow-up. Run: `npm run build && npm run test:e2e -w client`.
- **Accessibility:** `client/e2e/a11y.spec.ts` runs **axe** over home/login/signup/pricing/QR + the authenticated dashboard and fails on serious/critical violations (currently 0). Two reusable fixes to keep in mind: brand color in the logo uses `text-indigo-300`/`text-indigo-600` (the `primary` token fails contrast on both dark and light), and recharts pie sectors carry an unnamed `role="img"` — wrap pies in `DecorativeChart` (`components/ui/chart-a11y.tsx`), which labels the chart and marks sectors decorative via MutationObserver (a plain layout effect misses sectors that mount after ResponsiveContainer measures).
